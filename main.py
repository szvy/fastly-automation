...
with open(exportfile, "a") as f:
    f.write("links made with the fastly automation tool by https://github.com/szvy" + "\n" + "\n")
    f.write(firstlink + "\n")
...
def create_mailtm_account():
    ...
    session.post(f"{MAILTM_BASE}/accounts", json={"address": email, "password": password})
    token_response = session.post(f"{MAILTM_BASE}/token", json={"address": email, "password": password})
...
try:
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, "FirstName"))).send_keys("ScriptBy")
    time.sleep(1)
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, "LastName"))).send_keys("Szvy")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, "Company"))).send_keys("szvydotwin")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "Role"))).send_keys("DevOps - Leadership")
    WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.ID, "Email"))).send_keys(email)
    WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.ID, "Password"))).send_keys(password)

    print("fastly is verifying your password (may take a moment)")
    time.sleep(2)

    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[.//span[text()='Create a developer account']]"))
    ).click()
    ...
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[normalize-space()='Create service']"))
    ).click()
    input_field = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "serviceName")))
    input_field.clear()
    input_field.send_keys("githubdotcomslashszvy")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "domain"))).send_keys(domainfromdagen + ".global.ssl.fastly.net")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "host"))).send_keys(reversegoon)
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[.//span[text()='Activate']]"))
    ).click()
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Edit configuration')]"))
    ).click()
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Clone version 1 (active) to edit')]"))
    ).click()
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "Name"))).send_keys("sizzle")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "TTL"))).send_keys("0")
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "StaleTtl"))).send_keys("0")
    cacheshit = Select(driver.find_element(By.NAME, "action"))
    cacheshit.select_by_visible_text("Pass")
    cacheshit.select_by_value("pass")
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[@type='submit' and normalize-space(text())='Create']"))
    ).click()
    # link creation loop
    ...
    for _ in range(linksintvers):
        ...
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Create domain')]"))
        ).click()
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.NAME, "domain-name"))
        ).send_keys(domainfromdagen + ".global.ssl.fastly.net")
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(@class, 'primary') and contains(., 'Add')]"))
        ).click()
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "service-name"))).send_keys("githubdotcomslashszvy")
    WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Purge all')]"))
    ).click()
    WebDriverWait(driver, 10).until(
    ...
    input("thank you for using the fastly automation tool! your links have been exported to " + exported)
